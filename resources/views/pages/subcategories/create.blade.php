        ...
        <div class="-section">
            <div class="section-heading align-in-row-left">
                <div class="back-button">
                    <a href="{{url('/subcategories')}}"><i class='bx bxs-chevron-left' ></i></a>
                </div>
                <div class="section-title">
                    <h2>{{__('Subcategory Information')}}</h2>
                </div>
            </div>
            <hr>
            <div class="">
                <form id="subcategory_store_form" method="POST" action="{{route('subcategories.store')}}" enctype="multipart/form-data">
                    @csrf
                    <!-- Profile -->
                    <div class="">
                        {{-- <form> --}}
                            ...
                            <div class="row form-section">
                                <div class="col-md-6">
                                    <label for="post-title">{{__('Subcategory Title')}} <span class="mandatory_indicator">*</span></label>
                                    <input type="text" id="post-title" class="form-control" name="name" placeholder="Subcategory Title">
                                </div>
                                <div class="col-md-6">
                                    <label for="post-title">{{__('Category')}} <span class="mandatory_indicator">*</span></label>
                                    <select class="form-select" aria-label="Default select example" name="category_id">
                                        @if (count($categories) > 0)
                                            ...
                                            @endforeach
                                        @else
                                            <option selected disabled>{{__('No Categories found')}}</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row form-section mt-3">
                                <div class="col-md-4">
                                    <label for="subcategory-img-input">{{__('Subcategory Image')}} <span class="mandatory_indicator">*</span></label>
                                    <input id="subcategory-img-input" class="form-control" type="file" name="subcategory_cover_image">
                                </div>
                                <div class="col-md-4 text-center">
                                    <label for="selected-subcategory-img">{{__('Selected Image')}}</label>&nbsp;
                                    <img id="selected-subcategory-img" class="rounded-circle" style="background: #fff; width: 120px; height: 90px; object-fit: cover; border-radius: 10px !important;" src="{{asset('/img/system_default/default-image-placeholder.jpg') }}" alt="">
                                </div>
...
<script>
    function readURL(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                $('#selected-subcategory-img').attr('src', e.target.result);
            }
    ...
    $("#subcategory-img-input").change(function(){
        readURL(this);
    });
</script>

    {!! JsValidator::formRequest('App\Http\Requests\StoreSubcategoryRequest', '#subcategory_store_form'); !!}
@endpush
